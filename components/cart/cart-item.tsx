...
export function CartItemCard({ item, onCloseCart }: CartItemProps) {
  ...
  const merchandiseUrl = createUrl(`/product/${merchandise.product.handle}`, merchandiseSearchParams);

  const colorOption = merchandise.selectedOptions.find((option) => option.name.toLowerCase() === "color");
  const sizeOption = merchandise.selectedOptions.find((option) => option.name.toLowerCase() === "size");
      ...
      <div className="flex flex-row gap-6">
        ...
        <div className="flex flex-col gap-2 2xl:gap-3 flex-1">
          <Link
            ...
            prefetch
          >
            <span className="2xl:text-lg font-semibold">{merchandise.product.title}</span>
            {sizeOption && <span className="text-sm text-muted-foreground">Size: {sizeOption.value}</span>}
          </Link>
          <p className="2xl:text-lg font-semibold">₹{Number(item.cost.totalAmount.amount).toFixed(2)}</p>
